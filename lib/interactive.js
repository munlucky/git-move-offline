...
class Interactive {
  ...
  static printBox(title, content, width = 60) {
    ...
    const titlePadding = ' '.repeat(Math.max(0, width - 4 - getDisplayWidth(title)));
    ...
    contentLines.forEach(item => {
        const itemPadding = ' '.repeat(Math.max(0, width - 4 - getDisplayWidth(item)));
  ...
  static printTable(headers, rows) {
    const colWidths = headers.map((h, i) => {
      ...
      const maxContentWidth = Math.max(0, ...rows.map(r => getDisplayWidth(String(r[i] || ''))));
