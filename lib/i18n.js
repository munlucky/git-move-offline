...
const messages = {
  en: {
    ...
    depthOption: 'Export only recent N commits (shallow)',
    creatingShallowClone: 'Creating shallow clone (depth: %s)',
    shallowExportWarning: '⚠ Shallow export: Full history not included',
    ...
    branchExists:
      'Branch "%s" does not exist locally. What would you like to do?',
    ...
    resolveStep4:
      '  4. After resolving, cleanup: rm -rf .git-import-temp && git remote remove git-import-temp',
    conflictKeepResources:
      'Note: Remote and temporary files are preserved for conflict resolution.',
    ...
    branchMappingMode:
      'Branch Mapping Mode - Select target branch for each source branch',
    ...
    mapping: '  %s → %s',
    targetBranchNotExist:
      "Target branch '%s' does not exist. It will be created.",
    mergingInto: 'Merging %s into %s',
    allowUnrelatedOption: 'Allow merging branches with unrelated histories',
    allowUnrelatedHistories:
      'The branches have unrelated histories. Allow merge anyway?',
    unrelatedHistoriesNote:
      'Note: Merging unrelated histories (common in offline sync scenarios)',
  },
  ko: {
    ...
    depthOption: '최근 N개 커밋만 내보내기 (shallow)',
    creatingShallowClone: 'Shallow clone 생성 중 (depth: %s)',
    shallowExportWarning:
      '⚠ Shallow export: 전체 히스토리가 포함되지 않습니다',
    ...
    notGitRepoImport:
      'Git 저장소가 아닙니다. Git 프로젝트 디렉토리에서 실행하세요.',
    emptyRepo: '빈 저장소 감지됨',
    ...
    uncommittedChangesImport:
      '커밋되지 않은 변경사항이 있습니다. 먼저 커밋하거나 스태시하세요.',
    ...
    branchExists:
      '브랜치 "%s"가 로컬에 존재하지 않습니다. 어떻게 하시겠습니까?',
    createAndCheckout: '생성하고 체크아웃',
    skip: '건너뛰기',
    skipped: '건너뜀: %s',
    createdAndCheckedOut: '브랜치를 생성하고 체크아웃했습니다: %s',
    wouldCreateAndCheckout:
      '[드라이 런] 브랜치를 생성하고 체크아웃할 것입니다: %s',
    ...
    resolveStep3: '  3. git commit',
    resolveStep4:
      '  4. 해결 후 정리: rm -rf .git-import-temp && git remote remove git-import-temp',
    conflictKeepResources:
      '참고: 충돌 해결을 위해 리모트와 임시 파일이 유지됩니다.',
    ...
    notPushed:
      '변경사항이 푸시되지 않았습니다. 나중에 수동으로 푸시할 수 있습니다.',
    ...
    branchMappingMode:
      '브랜치 매핑 모드 - 각 소스 브랜치의 타겟 브랜치를 선택하세요',
    ...
    allowUnrelatedHistories:
      '브랜치들이 관계없는 히스토리를 가지고 있습니다. 그래도 병합하시겠습니까?',
    unrelatedHistoriesNote:
      '참고: 관계없는 히스토리 병합 중 (오프라인 동기화에서 일반적)',
  },
};
