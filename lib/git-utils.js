...
class GitUtils {
  ...
  execGit(command, options = {}) {
    try {
      ...
      // Prioritize stderr, but include stdout if it contains useful info
  ...
  mergeBranch(branch, options = {}) {
    ...
    const allowUnrelated = options.allowUnrelatedHistories
      ? '--allow-unrelated-histories'
      : '';

    try {
      this.execGit(
        `merge ${noFf} ${allowUnrelated} ${message} ${branch}`.trim()
      );
  ...
  createShallowClone(targetPath, depth, branches = null) {
    ...
    if (branches && Array.isArray(branches)) {
      ...
      branchArgs = branches.map((b) => `--branch ${b}`).join(' ');
    } else if (branches) {
      // Single branch
      branchArgs = `--branch ${branches}`;
    } else {
      // All branches
      branchArgs = '--no-single-branch';
    }

    // Use file:// protocol for local shallow clone
    const absolutePath = path.resolve(this.repoPath);
    this.execGit(
      `clone --depth ${depth} ${branchArgs} "file://${absolutePath}" "${targetPath}"`
    );
    return targetPath;
  }
}
