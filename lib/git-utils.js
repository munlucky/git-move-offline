...
class GitUtils {
  ...
  execGit(command, options = {}) {
    try {
      ...
      throw new Error(`Git command failed (exit ${exitCode}): ${command}\n\n${errorMsg}`);
    }
  }
  ...
  mergeBranch(branch, options = {}) {
    ...
    const allowUnrelated = options.allowUnrelatedHistories ? '--allow-unrelated-histories' : '';

    try {
      this.execGit(`merge ${noFf} ${allowUnrelated} ${message} ${branch}`.trim());
