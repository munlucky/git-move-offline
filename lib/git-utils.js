...
class GitUtils {
  ...
  getCommitCount() {
    try {
      const count = this.execGit('rev-list --count HEAD', { silent: true, allowError: true });
  ...
  getLocalBranches() {
    const output = this.execGit('branch --format="%(refname:short)"', { allowError: true });
    if (!output) return [];
    return output.split('\n').filter(b => b);
  }

  getRemoteBranches(remote = 'origin') {
    const output = this.execGit(`branch -r --format="%(refname:short)"`, { allowError: true });
    if (!output) return [];
    return output.split('\n')
  ...
  getConflicts() {
    try {
      const output = this.execGit('diff --name-only --diff-filter=U', { silent: true });
      return output ? output.split('\n').filter(f => f) : [];
    } catch {
      return [];
    }
